...
import {
  loadSettings, priceForDuration, extrasTotal, formatSoles,
...
export function BuyCreditsModal({ open, onClose, creditCost, currentBalance, onPurchaseComplete }: Props) {
  ...
  // 1 crédito = 1 sol → los créditos a comprar son el total en soles.
  const creditsToBuy = Math.round((packageBase + extrasSum) * 100) / 100;
  ...
  const handleBuy = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: `${creditsToBuy.toFixed(2)} créditos añadidos. Comprobante: ${invoiceNumber}`,
      });
      ...
      <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        {creditCost > 0 && (
          <div className="text-xs border p-3 bg-muted/30 flex items-center justify-between gap-2">
            <span className="text-muted-foreground">
              Para publicar tu aviso necesitas <b className="text-foreground">{creditCost.toFixed(2)} cr</b>
              {deficit > 0 && <> · tu saldo: {currentBalance.toFixed(2)} cr</>}
            </span>
            {deficit > 0 && <span className="font-bold text-destructive whitespace-nowrap">Faltan {deficit.toFixed(2)}</span>}
          </div>
        )}
            ...
            {DURATIONS.map((d) => {
              ...
              return (
                <button key={d} type="button" onClick={() => setDuration(d)}
                  ...
                  <p className="text-[11px] text-muted-foreground">{formatSoles(p)}</p>
                </button>
              );
            })}
          </div>
            ...
            {EXTRA_DEFS.map((d) => {
              ...
              return (
                <button key={d.key} type="button"
                  ...
                  <p className="text-xs font-semibold text-secondary mt-1">+{formatSoles(unit)}</p>
        ...
        <div className="border p-3 bg-secondary/5 space-y-1">
          <div className="flex justify-between text-xs">
            ...
            <span className="font-semibold">{formatSoles(packageBase)}</span>
          </div>
          {extrasSum > 0 && (
            <div className="flex justify-between text-xs">
              <span className="text-muted-foreground">Adicionales</span>
              <span className="font-semibold">{formatSoles(extrasSum)}</span>
            </div>
          )}
          <div className="border-t pt-2 flex justify-between items-baseline">
            <span className="font-bold uppercase tracking-wider text-xs">Créditos a comprar</span>
            <span className="text-2xl font-extrabold text-secondary">{creditsToBuy.toFixed(2)} cr</span>
          </div>
          {creditCost > 0 && (
            <p className={`text-[11px] ${coversAd ? "text-success" : "text-destructive"}`}>
              {coversAd
                ? "✓ Con esta compra podrás publicar tu aviso."
                : `Aún faltarían ${(creditCost - balanceAfter).toFixed(2)} cr para publicar tu aviso.`}
            </p>
          )}
        </div>
        ...
        <DialogFooter className="gap-2 pt-2">
          ...
          <Button onClick={handleBuy} disabled={buying || creditsToBuy <= 0} className="gap-2">
            {buying
              ...
              : <><ShieldCheck size={14} /> Comprar {formatSoles(creditsToBuy)}</>}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
